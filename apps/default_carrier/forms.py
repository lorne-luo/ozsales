# coding=utf-8
from django import forms
from core.django.forms import NoManytoManyHintModelForm
from .models import DefaultCarrier


class DefaultCarrierAddForm(NoManytoManyHintModelForm):
    website = forms.URLField(label='官网地址', max_length=30, required=True, help_text='官方网站地址')  # make mandatory

    class Meta:
        model = DefaultCarrier
        fields = ['name_cn', 'name_en', 'website', 'search_url', 'id_upload_url']


class DefaultCarrierDetailForm(NoManytoManyHintModelForm):
    class Meta:
        model = DefaultCarrier
        fields = ['name_cn', 'name_en', 'website', 'search_url', 'post_search_url', 'id_upload_url', 'track_id_regex',
                  'is_default']


class DefaultCarrierUpdateForm(DefaultCarrierAddForm):
    pass


class DefaultCarrierAdminForm(DefaultCarrierUpdateForm):
    class Meta:
        model = DefaultCarrier
        fields = ['name_cn', 'name_en', 'website', 'search_url', 'post_search_url', 'id_upload_url', 'track_id_regex',
                  'is_default']

