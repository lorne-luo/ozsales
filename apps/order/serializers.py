...
class OrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = ['link'] + ['customer', 'address', 'is_paid', 'status', 'total_amount', 'product_cost_aud', 'product_cost_rmb', 'shipping_fee', 'ship_time', 'total_cost_aud', 'total_cost_rmb', 'origin_sell_rmb', 'sell_price_rmb', 'profit_rmb', 'finish_time'] + ['id']
        read_only_fields = ['id']
    ...
    def get_link(self, obj):
        ...
        if request.user.has_perm(change_perm_str):
            ...
            url = reverse('order:order-detail', args=[obj.id])
