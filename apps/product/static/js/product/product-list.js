(function(self, $, undefined) {
  ...
  self.delete = function($row, pk) {
    ...
    return function(event){
      $.ajax({
        dataType: 'json',
        url: '/api/product/product/' + pk + '/',
        type: 'DELETE',
        headers: {"X-CSRFToken": $.cookie('csrftoken')},
        complete: self.delete_product_callback($row)
      });
    };
  };

  self.delete_product_callback = function($row) {
...
}(window.OZProductList = window.OZProductList || {}, jQuery));
/**
 * On ready.
 */
$(document).ready(function() {
  OZProductList.initialize();
});
