var App = function () {
    ...
    var handleHinter = function(){
        if ($("#page-hint .content").length == 0){
            $("#page-hint").hide();
        } else {
            jQuery('#page-hint').click(function () {
                if ($(this).attr("opened") && !$(this).attr("opening") && !$(this).attr("closing")) {
                    $(this).removeAttr("opened");
                    $(this).attr("closing", "1");
                    $("#page-hint .content").hide('fadeOut');
                    $("#page-hint").css("overflow", "hidden").animate({
                        width: '20px',
                        backgroundColor: '#25c3cf'
                    }, {
                        complete: function () {
                            $(this).removeAttr("closing");
                            $("#page-hint i.icon-remove").removeClass('icon-remove').addClass('icon-question-sign');

                        }
                    });
                } else if (!$(this).attr("closing") && !$(this).attr("opening")) {
                    $(this).attr("opening", "1");
                    $("#page-hint").css("overflow", "visible").animate({
                        width: '350px',
                        ...
                        complete: function () {
                            ...
                            $(this).attr("opened", 1);
                            $("#page-hint .content").show('fadeIn');
                            $("#page-hint i.icon-question-sign").removeClass('icon-question-sign').addClass('icon-remove');
                        }
                    });
    ...
    var handleTables = function (table) {
        if (!jQuery().dataTable) {
            return;
        }
        var _table = '.table-list';
        if (table !== undefined){
            _table = table;
        }

        // Add dd/mm/yy date Sorting (source: http://jsfiddle.net/MrTest/yM24R/)
        ...
        // Sorting based on alt attribute, has to be like: <span alt='2014020227'>today</span>
         ...
         */
        jQuery.fn.dataTableExt.oApi.fnSortOnOff  = function ( oSettings, aiColumns, bOn ) {
        ...
        // Depricated - please do no use, use class="table-list" instead
        ...
        jQuery('#table_list .group-checkable').change(function () {
        ...
        jQuery('#table_unsorted_list .group-checkable').change(function () {
            var set = jQuery(this).attr("data-set");
            var checked = jQuery(this).is(":checked");
            jQuery(set).each(function () {
                if (checked) {
        ...
        jQuery('#table_list_wrapper .dataTables_filter input').addClass("input-medium"); // modify table search input
        jQuery('#table_list_wrapper .dataTables_length select').addClass("input-mini"); // modify table per page dropdown
        */// End deprication

        $('input:checkbox', _table).uniform();
        ...
        if($.fn.dataTable.moment){
            //add all time format used in our project
        ...
        $('.group-checkable', _table).change(function () {
            var nodes = $(_table).DataTable().rows().nodes().flatten().toArray();
            if ($(this).is(':checked')){
                $('input:checkbox:first', nodes).attr('checked', true);
            } else {
                $('input:checkbox:first', nodes).attr('checked', false);
            }
            $.uniform.update($('input:checkbox:first', nodes));
        });
    };

    var handleDateTimePickers = function (from_now) {

        if (jQuery().datetimepicker) {
            var now = new Date();
            var now_minute=new Date(now.getFullYear(),now.getMonth(),now.getDate(),now.getHours(),now.getMinutes());

            var datetimepicker_setting={
                dateFormat: "dd/mm/yy",
                timeFormat: "hh:mm TT",
                showSecond: false,
            ...
            if(from_now){
                datetimepicker_setting['minDateTime'] = now_minute;
            }

            $('.datetimepicker').datetimepicker(datetimepicker_setting);

            $(".datetimepicker_trigger .trigger").click(function(e){
                var input = $(e.target).closest( '.datetimepicker_trigger' ).find('input.datetimepicker');
                if (input) input.datetimepicker("show");
            });
            ...
            $(".datepicker_trigger .trigger").click(function(e){
                var input = $(e.target).closest( '.datepicker_trigger' ).find('input.datepicker');
                if (input) input.datetimepicker("show");
            });
            ...
            if(from_now){
                timepicker_setting['minDateTime'] = now_minute;
            }

            $('.timepicker').timepicker(timepicker_setting);

            $(".timepicker_trigger .trigger").click(function(e){
                var input = $(e.target).closest( '.timepicker_trigger' ).find('input.timepicker');
                if (input) input.datetimepicker("show");
            });

            $('.datetimepicker-group .remove, .timepicker-group .remove').click(function(e){
                e.preventDefault();
                var input = $(e.target).closest('.controls').find('input');
                if (input) input.val('');
            });
        ...
        if(from_now){
            timepicker24_setting['minDateTime'] = now_minute;
        }

        $('.timepicker-24').timepicker(timepicker24_setting);
    }
    ...
    return {
        ...
        init: function () {
            ...
            //for momentjs bug, default lang is zh-tw after loaded
            if(moment){
            ...
            handleHinter(); // handles style customer tool
...
//                handleIntro();
        ...
        initDateTime: function(from_now){
            from_now = from_now || false;
            handleDateTimePickers(from_now);
        },
...
// Needed to wait for an array of ajax requests to all finish
// http://stackoverflow.com/questions/5627284/pass-in-an-array-of-deferreds-to-when?rq=1
