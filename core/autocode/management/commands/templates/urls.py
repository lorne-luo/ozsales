...
urlpatterns += patterns('',
    ...
    url(r'^<% app_name %>/<% model_name %>/(?P<pk>\d+)/$', views.<% MODEL_NAME %>DetailView.as_view(), name='<% model_name %>-detail'),
    url(r'^<% app_name %>/<% model_name %>/(?P<pk>\d+)/edit/$', views.<% MODEL_NAME %>UpdateView.as_view(), name='<% model_name %>-update'),
)
